...
// RequestOption is an option for the requests made by the AIDR API Client
// which can be supplied to clients, services, and methods. You can read more about this functional
// options pattern in our [README].
//
// [README]: https://pkg.go.dev/github.com/crowdstrike/aidr-go#readme-requestoptions
type RequestOption = requestconfig.RequestOption

// WithBaseURLTemplate returns a RequestOption that sets the BaseURLTemplate for the client.
// The template may contain {SERVICE_NAME} placeholders that will be replaced with the
// service name slug at request-time.
//
// For security reasons, ensure that the base URL template is trusted.
func WithBaseURLTemplate(baseURLTemplate string) RequestOption {
	return requestconfig.RequestOptionFunc(func(r *requestconfig.RequestConfig) error {
		r.BaseURLTemplate = baseURLTemplate
		return nil
	})
}

// WithServiceName returns a RequestOption that sets the service name for template replacement.
// This service name will be used to replace {SERVICE_NAME} placeholders in BaseURLTemplate.
func WithServiceName(serviceName string) RequestOption {
	return requestconfig.RequestOptionFunc(func(r *requestconfig.RequestConfig) error {
...
// WithToken returns a RequestOption that sets the client's API token.
// Service-specific API tokens will take precedence over this.
...
// WithServiceToken returns a RequestOption that sets a service-specific API
// token. Service-specific API tokens override the client-level API token when
// present.
func WithServiceToken(serviceName, token string) RequestOption {
	return requestconfig.RequestOptionFunc(func(r *requestconfig.RequestConfig) error {
		r.ServiceTokens.Store(serviceName, token)
		return nil
	})
}
